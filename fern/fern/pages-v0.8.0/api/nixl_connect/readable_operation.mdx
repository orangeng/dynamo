...
The NIXL metadata ([RdmaMetadata](rdma_metadata)) required to access the memory referenced by the provided descriptors is accessible via the operations `.metadata()` method.
...
Generates and returns the NIXL metadata ([RdmaMetadata](rdma_metadata)) required for a remote worker to read from the operation.
Once acquired, the metadata needs to be provided to a remote worker via a secondary channel, most likely HTTP or TCP+NATS.
...
Returns [`OperationStatus`](operation_status) which provides the current state (aka. status) of the operation.
